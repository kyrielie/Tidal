...
public record WaveResourceMetadata(int frameTime, int frameWidth, int frameHeight) {
    public static final String KEY = "wave_animation";

    public static final WaveResourceMetadata DEFAULT = new WaveResourceMetadata(5, 16, 16);
    public static final Codec<WaveResourceMetadata> CODEC = RecordCodecBuilder.create((instance) ->
            instance.group(
                    Codecs.POSITIVE_INT.optionalFieldOf("frametime", 5).forGetter(WaveResourceMetadata::frameWidth),
                    Codecs.POSITIVE_INT.optionalFieldOf("frame_width", 16).forGetter(WaveResourceMetadata::frameWidth),
